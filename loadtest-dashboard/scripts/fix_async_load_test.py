...
def save_summary_json(results: List[UserResult], elapsed_s: float, config: TestConfig, output_file: str = "results.json") -> None:
    ...
    # Ensure output directory exists (relative to the script now)
...
def parse_args() -> TestConfig:
    parser = argparse.ArgumentParser(description="Async FIX load test")
    ...
    # Default to local public folder relative to the dashboard root inside container
    parser.add_argument("--output-file", default="public/loadtest-results.json")
